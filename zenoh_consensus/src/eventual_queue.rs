...
impl EventualQueue {
    ...
    pub async fn new(
        ...
        let pop_key =
            Arc::new(zenoh::Path::try_from(format!("{}/{}", pop_dir, id)).unwrap());
                            ...
                            match &mut *state {
                                        ...
                                        *deque_return = None;
                                        ...
                                        if change.path == *request_key {
                                            ...
                                            *state = State::Requesting {
                                            ...
                                            let peer_id: usize =
                                                change.path.last_segment().parse().unwrap();
                                            let request_item = format!("{:?}", change.value).parse::<usize>().unwrap();
                                            unattended_requests.push((peer_id, change.timestamp, request_item));
                                        }
                                    }
                                ...
                                State::Requesting {
                                	...
                                	if unattended_requests.len() != 0 {
                                        ...
                                        earlier_same_request = unattended_requests.into_iter().filter(|(_id, timestamp_unattended, req_item)| *timestamp_unattended < *timestamp && *req_item == *to_pop).map(|(id, _timestamp, _req_item)| *id).collect::<Vec<usize>>();
                                		...
                                		let to_ack = unattended_requests.into_iter().filter(|(_id, timestamp_unattended, req_item)| *timestamp_unattended < *timestamp || *req_item != *to_pop).map(|(id, _timestamp, _req_item)| *id).collect::<Vec<usize>>();
                                    ...
                                    if change.timestamp < *timestamp {
                                        ...
                                        eprintln!("peer {} sends ack to {}", id, ack_key);
                                        let request_item = format!("{:?}", change.value).parse::<usize>().unwrap();
                                        if request_item == *to_pop || earlier_same_request.len() != 0 {
                                            ...
                                            if queue.len() > *idx + 1{
                                                zenoh
                                                    .workspace(None)
                                                    .await?
                                                    .put(&request_key, format!("{}", queue[*idx+1]).into())
                                                    .await?;
                ...
                let ack_future = workspace
                    ...
                    .try_for_each(|change| {
                        ...
                        async move {
                            ...
                            eprintln!("peer {} received {} from {}", id, req_type.unwrap(), change.path);
                            let mut state = state.lock().await;
                            ...
                            match &mut *state {
                                State::Requesting {
                                    ...
                                    if req_type.unwrap() == 0{ // Ack Request
                                        *ack_count += 1;
                                    }
                                        ...
                                        if to_pop != None{
                                            *state = State::Pop {
                                            ...
                                            zenoh
                                                .workspace(None)
                                                .await?
                                                .put(&pop_key, zenoh::Value::Integer(to_pop.unwrap() as i64))
                                                .await?;
                ...
                let enqueue_future = workspace
                    ...
                    .try_for_each(|change| {
                        ...
                        async move {
                            ...
                            eprintln!("peer {} received enqueue item = {}", id, format!("{:?}", item.unwrap()));
                            let mut queue = queue.lock().await;
                            let not_popped = not_popped.lock().await;

                            let request_item = item.unwrap() as usize;
                            let index = not_popped.iter().position(|value| *value == request_item);
                            if index == None{
    ...
    pub async fn dequeue(&self)->Option<usize> {
        ...
        let mut non_flag = false;
        {
            ...
            *state = State::PreRequest{
            ...
            if to_pop != None{
                workspace.put(request_key, zenoh::Value::Integer(to_pop.unwrap() as i64)).await.ok()?;
                eprintln!("peer {} sends request {} to {}", id, to_pop.unwrap(), request_key);
            }
    ...
    pub async fn enqueue(&self, item:usize){
        ...
        queue.push(item);
        workspace.put(enqueue_key, zenoh::Value::Integer(item as i64)).await.unwrap();
