...
async fn send_request(whole_request_dir: &str){
    ...
    workspace.put(
        &whole_request_dir.try_into().unwrap(),
        "Request".into()
    ).await.unwrap();
    zenoh.close().await.unwrap();
}

async fn change_req_loop(id:i32, request_dir: &str, to_lock: Arc<Mutex<bool>>, req_time_set: Arc<Mutex<bool>>, request_lst: Arc<Mutex<Vec::<i32>>>){
    //println!("Debug!");
    ...
    while let Some(change) = change_stream_request.next().await {
        println!("[Subscription listener]  {:?} for {} : {:?} at {}",
...
impl LamportMutex {
    ...
    pub fn start(&self){
        ...
        let handler = thread::spawn(move || LamportMutex::begin(id, whole_request_dir, to_lock, req_time_set, request_lst));
    }

    pub fn begin(id:i32, whole_request_dir:String, to_lock:Arc<Mutex<bool>>, req_time_set:Arc<Mutex<bool>>, request_lst:Arc<Mutex<Vec::<i32>>>){
        let rt = Runtime::new().unwrap(); 
        rt.spawn(async move{ change_req_loop(id, &whole_request_dir, to_lock, req_time_set, request_lst).await });
        loop{};
